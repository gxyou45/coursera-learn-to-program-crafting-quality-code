...
def build_rating_list(name_to_rating, names_final):
    ...
    for name in names_final:
        rank_list.append([name_to_rating[name], name])
    rank_list.sort()
    rank_list.reverse()
    return rank_list
...
def filter_by_cuisine(names_matching_price, cuisine_to_names, cuisines_list):
    ...
    for cuis in cuisines_list:
        for name in names_matching_price:
            if name in cuisine_to_names[cuis] and not (name in cuisines):
...
def read_restaurants(file):
    ...
    for rest in restaurant_list:
        rest[1] = int(rest[1][:-1])
        rest[3] = rest[3].split(',')
        name_to_rating.update({rest[0]:rest[1]})
        price_to_names[rest[2]].append(rest[0])
        for cuis in rest[3]:
            if cuis in cuisine_to_names.keys():
                cuisine_to_names[cuis].append(rest[0])
            else:
                cuisine_to_names.update({cuis: [rest[0]]})
